...
#define PanSource	"lock_env2.pml"
...
#define _nstates2	53	/* main_control */
#define minseq2	175
#define maxseq2	226
#define _endstate2	52
...
struct valves_t { /* user defined type */
	uchar lower;
	uchar higher;
};
...
#define Air0	(sizeof(P0) - Offsetof(P0, lockid2) - 1*sizeof(uchar))
...
typedef struct State {
	...
	uchar request_west;
	uchar request_east;
	uchar observed_west[3];
	uchar observed_east[3];
	uchar change_doors_pos;
	uchar doors_pos_changed;
	uchar change_valve_pos;
	uchar valve_pos_changed;
	uchar LOCK_ORIENTATION[3];
	uchar ship_status[2];
	struct doorpairs_t doors_status[3];
	struct valves_t valve_status[3];
#ifdef TRIX
